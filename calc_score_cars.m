function [Score_exp, Score_order] = calc_score_cars(lambda, Option, NumOfCars, beta, alfa, p)
...
for i = 1:length(rows)
    if ~isempty(NumOfCars(rows(i), cols(i)))
...
%     if ~isempty(NumOfCars(rows_other(i), cols_other(i)))
